...
def main():
    ...
    # load data from file
    parser.add_argument('--data_path', type=str, default='/home/tanisha/mapFEAT/TrainImages/', help='contains JSON and Map Image .tif file')
    parser.add_argument('--prediction', type=str, default='/home/tanisha/mapFEAT/results/', help='contains predicted files')
    parser.add_argument('--gtlabel', type=str, default='/home/tanisha/mapFEAT/gt_labels/', help='contains gt labels')
    
    args = parser.parse_args()
    
    pred = os.listdir(args.prediction)
    labels = []

    for each in pred:
        labels.append(each.strip('.jpeg'))
 
    for gt,res in zip(labels,pred) : 
        pred_loc  = os.path.join(args.prediction,res)
        file_path = os.path.join(args.gtlabel,gt+'.tif')   
        mat_img = cv2.imread(file_path)
        mat_pred  = cv2.imread(pred_loc)
        
        precision, recall, f_score=feature_f_score(args.data_path, args.prediction, args.gtlabel,
